...
// ─── Payload Validation ────────────────────────────────────────────────
const REQUIRED_FIELDS = ["title", "description", "requesterName", "requesterId", "requestId"];

const validateJobData = (jobData) => {
    const missing = REQUIRED_FIELDS.filter((f) => jobData[f] === undefined || jobData[f] === null);
    if (missing.length > 0) {
        throw new Error(`Email job missing required fields: ${missing.join(", ")}`);
    }
    if (typeof jobData.requestId !== "number" || jobData.requestId <= 0) {
        throw new Error(`Invalid requestId: ${jobData.requestId}`);
    }
    if (typeof jobData.requesterId !== "number" || jobData.requesterId <= 0) {
