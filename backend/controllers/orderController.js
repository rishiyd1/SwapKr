import { BuyRequest, Item, Chat, Message } from "../models/index.js";
...
export const sendBuyRequest = async (req, res) => {
    try {
        ...
        if (item.sellerId === buyerId) {
            return res
                .status(400)
                .json({ message: "You cannot buy your own item" });
        }
        ...
        if (io) {
            ...
            const sellerSocketId = onlineUsers.get(item.sellerId);
            if (sellerSocketId) {
                io.to(sellerSocketId).emit("new_buy_request", fullRequest);
            }
        }
